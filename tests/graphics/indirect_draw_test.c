...
PalBool indirectDrawTest()
{
    ...
    window = gfxHelperCreateWindow("Indirect Draw Window", eventDriver);
    if (!window) {
        return PAL_FALSE;
    }
    ...
    void* ptr = nullptr;
    result = palMapBuffer(stagingBuffer, 0, stagingBufferSize, &ptr);
    if (result != PAL_RESULT_SUCCESS) {
    ...
    while (running) {
        ...
        palCmdBindIndexBuffer(cmdBuffers[currentFrame], indexBuffer, 0, PAL_INDEX_TYPE_UINT32);
        palCmdDrawIndexedIndirect(cmdBuffers[currentFrame], indirectBuffer, 1);
