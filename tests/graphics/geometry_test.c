...
bool geometryTest()
{
    ...
    windowCreateInfo.title = "Geometry Window";
    ...
    for (Int32 i = 0; i < adapterCount; i++) {
        ...
        if (hasGraphicsQueue) {
            ...
            if (adapterFeatures & PAL_ADAPTER_FEATURE_GEOMETRY_SHADER) {
    ...
    if (!adapter) {
        if (!hasGraphicsQueue) {
            ...
            palLog(nullptr, "Failed to find an adapter that supports geometry shader");
    ...
    if (adapterInfo.shaderFormats & PAL_SHADER_FORMAT_SPIRV) {
        sources[0] = "graphics/shaders/bin/spirv/geometry_vert.spv";
        sources[1] = "graphics/shaders/bin/spirv/triangle_frag.spv";
        sources[2] = "graphics/shaders/bin/spirv/geometry.spv";

    } else if (adapterInfo.shaderFormats & PAL_SHADER_FORMAT_DXIL) {
        sources[0] = "graphics/shaders/bin/dxil/geometry_vert.dxil";
        sources[1] = "graphics/shaders/bin/dxil/triangle_frag.dxil";
        sources[2] = "graphics/shaders/bin/dxil/geometry.dxil";
    }
