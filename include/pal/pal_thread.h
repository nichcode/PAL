 ...
 * 
 * @param[in] arg Optional pointer to user data passed into the function. Can be `nullptr`.
 * 
 * @return A pointer
 * 
 * @sa palCreateThread
 ...
 * 
 * @param[in] userData Optional pointer to user data passed into the function. Can be `nullptr`.
 * 
 * @sa palCreateThread
 * @ingroup thread
 */
...
typedef struct {
    ...
    void* arg;             /** Optional pointer to user data for thread function. Can be `nullptr`*/
} PalThreadCreateInfo;

/**
 * @brief Get the allocator for all thread allocations.
 * 
 * The allocator will be used for creating threads, mutexes, and condition variables.
 * 
 ...
 * 
 * The thread begins executing the entry function provided by the user in `info`.
 * The thread runs until the entry function has finished executing or its detached with `palDetachThread()`.
 * The `info` pointer must be valid, explicitly initialized by the user. see `PalThreadCreateInfo` struct.
 *
 * @param[in] info Pointer to the PalThreadCreateInfo struct with creation specifications.
 ...
 *
 * @note This function is thread safe.
 * @note Blocks the calling thread unti the provided thread has finished or destryoyed
 *
 * @sa palDetachThread()
 * @ingroup thread
 */
 ...
 * 
 * `PAL_THREAD_FEATURE_PRIORITY` must be supported otherwise, this function will fail and
 * `PAL_RESULT_THREAD_FEATURE_NOT_SUPPORTED` will be returned.
 * 
 ...
 * 
 * `PAL_THREAD_FEATURE_AFFINITY` must be supported otherwise, this function will fail and
 * `PAL_RESULT_THREAD_FEATURE_NOT_SUPPORTED` will be returned.
 *  Thread affinity is the number of CPU cores the thread is allowed to be executed on.
 * 
 * @param[in] thread The thread
 * 
 ...
 * The allocator used to allocate the string is the same passed in `palSetThreadAllocator()`.
 * 
 * `PAL_THREAD_FEATURE_NAME` must be supported otherwise, this function will fail and
 * `PAL_RESULT_THREAD_FEATURE_NOT_SUPPORTED` will be returned.
 * 
 * @param[in] thread The thread
 * 
 ...
 * 
 * @return @return `PAL_RESULT_SUCCESS` on success or an appropriate result code on failure.
 *
 * @note This function is not thread safe.
 *
 * @ingroup thread
