 ...
 *
 * Check after palInitVideo() to check which features are supported on the
 * platform (OS). Example: on Windows 7, `PAL_VIDEO_FEATURE_HIGH_DPI` is not
 * supported.
 *
 * @code
 ...
 * @endcode
 *
 * @note All video features follow the format `PAL_VIDEO_FEATURE_**` for
 ...
 *
 * @note All orientation follow the format `PAL_ORIENTATION_**` for consistency
 * and API use.
 *
 * @ingroup video
 */
 ...
 * This is a bitmask enum and multiple flags can be OR'ed together using bitwise
 * OR operator (`|`).
 *
 * @code
 ...
 * @endcode
 *
 * @note All window flags follow the format `PAL_WINDOW_STYLE_**` for
 * consistency and API use.
 *
 * @ingroup video
 */
 ...
 *
 * @note All state follow the format `PAL_WINDOW_STATE_**` for consistency and
 * API use.
 *
 * @ingroup video
 */
 ...
 *
 * This is a bitmask enum and multiple flags can be OR'ed together using bitwise
 * OR operator (`|`). If `PAL_FLASH_CAPTION` will be used,
 * `PAL_VIDEO_FEATURE_WINDOW_FLASH_CAPTION` must be supported.
 *
 * If `PAL_FLASH_TRAY` will be used, `PAL_VIDEO_FEATURE_WINDOW_FLASH_TRAY` must
 * be supported.
 *
 * Example:
 *
 ...
 * @endcode
 *
 * @note All flash types follow the format `PAL_FLASH_**` for consistency and
 * API use.
 ...
 *
 * @note All scancodes follow the format `PAL_SCANCODE_**` for consistency and
 * API use.
 *
 * @ingroup video
 */
 ...
 *
 * @note All keycodes follow the format `PAL_KEYCODE_**` for consistency and API
 * use.
 *
 * @ingroup video
 */
 ...
 *
 * @note All mouse buttons follow the format `PAL_MOUSE_BUTTON_**` for
 * consistency and API use.
 *
 * @ingroup video
 */
 ...
 * query supported features, create windows and other video related
 * functionality. This must called before any video related function.
 *
 * The video system must be shutdown with `palShutdownVideo()` when no longer
 * needed. The `eventDriver` must be valid to recieve video related events.
 *
 * @param[in] allocator Optional user provided allocator. Can be `nullptr` to
 * use default.
 * @param[in] eventDriver Optional event driver to push events to. If `nullptr`
 ...
 *
 * The video system must be initialized before this call otherwise this
 * functions returns silently. This function does not destroy created windows
 * and other video resources, therefore all created windows and other video
 * resources must be destroyed explicitly before this function.
 *
 * @note This function must be called from the main thread.
 *
 * @sa palInitVideo()
 ...
 *
 * The video system must be initialized before this call otherwise this
 * functions returns silently. This function also pushes generated video related
 * events, if a valid event driver was set at `palInitVideo()`.
 *
 * @note This function must be called from the main thread.
 *
 * @sa palInitVideo()
 ...
 * `monitors`. The user is responsible for the life time of the array.
 *
 ...
 * @endcode
 *
 * you can set the `monitors` to `nullptr` and PAL will set the number of the
 * connected monitors to `count`.
 *
 ...
 * If `count` is less than the number of connected monitors, PAL will write up
 * to `count`.
 *
 ...
 * failure.
 *
 * @note Users must not free the monitor handles, they are managed by the
 * platform (OS). Users are required to cache this, and call this function again
 * if monitors are added or removed. see `PAL_EVENT_MONITOR_LIST_CHANGED`
 *
 * @note This function must be called from the main thread.
 *
 ...
 * failure.
 *
 * @note Users must not free the monitor handle, it is managed by the platform
 * (OS).
 * @note This function must be called from the main thread.
 *
 ...
 *
 * Users must `allocate` statically or dynamically and pass the maximum capacity
 * of the allocated array as `count` and also pass the array itself as `modes`.
 ...
 * @endcode
 *
 * you can set the `modes` to `nullptr` and PAL will set the number of available
 * monitor modes to `count`.
 *
 * If the `count` is zero or less than zero and the `modes` is nut `nullptr`,
 * this function fails and returns `PAL_RESULT_INSUFFICIENT_BUFFER`.
 * If `count` is less than the available monitor modes, PAL will write up to
 * `count`.
 *
 * @param[in] monitor Monitor handle.
 ...
 * users must get the monitor mode from `palEnumerateMonitorModes()` or
 * `palValidateMonitorMode()` to validate the mode before changing.
 *
 * If the monitor mode submitted is invalid, this function might fail depending
 * on the platform (OS).
 *
 * `PAL_VIDEO_FEATURE_MONITOR_MODE_SWITCH` must be supported
 * otherwise this function fails and returns
 ...
 *
 * @param[in] monitor Monitor handle.
 * @param[in] orientation The monitor orientation. example:
 ...
 *
 * @note This function must be called from the main thread.
 * @note This change is temporary and will be reset when the platform (OS) is
 * reboot.
 *
 * @sa palGetPrimaryMonitor(), palEnumerateMonitors()
 * @ingroup video
 ...
 *
 * The video system must be initialized before this call.
 * The `info` pointer must be valid, explicitly initialized by the user. see
 * `PalWindowCreateInfo` struct.
 *
 * @param[in] info Pointer to the PalWindowCreateInfo struct with creation
 * specifications.
 ...
 * The video system must be initialized before this call.
 *
 * If the window is already minimized, this functions does nothing and returns
 * `PAL_RESULT_SUCCESS`. If `PAL_VIDEO_FEATURE_WINDOW_MINMAX` is not supported,
 ...
 * The video system must be initialized before this call.
 *
 * If the window is already maximized, this functions does nothing and returns
 * `PAL_RESULT_SUCCESS`. If `PAL_VIDEO_FEATURE_WINDOW_MINMAX` is not supported,
 * this function will fail and return `PAL_RESULT_VIDEO_FEATURE_NOT_SUPPORTED`.
 *
 * @param[in] window Pointer to the window.
 ...
 * The video system must be initialized before this call.
 *
 * If the window was previously maximized or minimized, this function reverts
 * state to `PAL_WINDOW_STATE_RESTORED`
 *
 * @param[in] window Pointer to the window.
 *
 * @return `PAL_RESULT_SUCCESS` on success or an appropriate result code on
 ...
 * supported.
 *
 * If a flash type is used and its not supported, this function will fail and
 ...
 * string is the same passed in `palInitVideo()`.
 *
 * @param[in] window Pointer to the window.
 ...
 *
 * @param[in] window Pointer to the window.
 * @param[out] x Pointer to recieve the window client position x. Can be
 * `nullptr`.
 * @param[out] y Pointer to recieve the window client position y. Can be
 * `nullptr`.
 *
 * @return `PAL_RESULT_SUCCESS` on success or an appropriate result code on
 * failure.
 ...
 * The video system must be initialized before this call.
 *
 * The returns either `PAL_WINDOW_STATE_MINIMIZED` or
 * `PAL_WINDOW_STATE_MAXIMIZED` or `PAL_WINDOW_STATE_RESTORED`, if the window is
 * minimzed or maximized or restored respectively.
 *
 * @param[in] window Pointer to the window.
 *
 * @return `PAL_RESULT_SUCCESS` on success or an appropriate result code on
 ...
 * `palUpdateInput()` is called. The array must be index with PalKeycodes and
 * not exceed `PAL_KEYCODE_MAX`. see `PalKeycode`
 *
...
/**
 * @brief Get the current state of the scancodes (layout independent keys) of
 * the keyboard.
 *
 * The video system must be initialized before this call.
 *
 * The returned pointer must not be freed. The state is updated when
 * `palUpdateInput()` is called. The array must be index with PalScancodes and
 * not exceed `PAL_SCANCODE_MAX`. see `PalScancode`
 *
 ...
 *
 * The returned pointer must not be freed. The state is updated when
 * `palUpdateInput()` is called. The array must be index with PalMouseButton and
 * not exceed `PAL_MOUSE_BUTTON_MAX`. see `PalMouseButton`
 *
 ...
 * @param[in] window Pointer to the window.
 *
 * @return True if the window is visible otherwise false. This will return false
 ...
 * The video system must be initialized before this call.
 *
 * @return The currently input focused window on success or `nullptr` on
 * failure.
 *
 * @note This function is thread-safe.
 *
 ...
 * users must not call any video function take takes in the `window` after
 * deletion.
 *
 * @param[in] window Pointer to the window.
 *
 ...
 * The video system must be initialized before this call.
 *
 * The window must have `PAL_WINDOW_STYLE_TRANSPARENT` style. The `opacity` must
 ...
 *
 * @note This function must be called from the main thread.
 * @note Some platforms (OS) with window managers may clapped or adjust the
 * size.
 *
 * @sa palCreateWindow(), palSetWindowPos()
 * @ingroup video
 ...
 * The video system must be initialized before this call.
 * The `info` pointer must be valid, explicitly initialized by the user. see
 * `PalIconCreateInfo` struct.
 *
 * @param[in] info Pointer to the PalIconCreateInfo struct with creation
 * specifications.
 ...
 *
 * The video system must be initialized before this call. Both the `window` and
 * the `icon` must be valid.
 *
 * @param[in] window Pointer to the window.
 ...
 * The video system must be initialized before this call.
 * The `info` pointer must be valid, explicitly initialized by the user. see
 * `PalCursorCreateInfo` struct.
 *
 * @param[in] info Pointer to the PalCursorCreateInfo struct with creation
 * specifications.
 ...
 * into a single one on the screen or desktop
 *
 ...
 *
 * @param[in] window Pointer to the window.
 * @param[out] x Pointer to recieve the window cursor's position x. Can be
 * `nullptr`.
 * @param[out] y Pointer to recieve the window cursor's position y. Can be
 * `nullptr`.
 *
 * @return `PAL_RESULT_SUCCESS` on success or an appropriate result code on
 * failure.
