 ...
 *
 * The opengl system must be initialized before this call otherwise this
 * functions returns silently. This function does not destroy created contexts.
 *
 * @note This function must be called from the main thread.
 *
 * @sa palInitGL()
 ...
 * `configs`. The user is responsible for the life time of the array.
 *
 ...
 * @endcode
 *
 * you can set the `configs` to `nullptr` and PAL will set the number of the
 * connected configs to `count`.
 *
 ...
 * If `count` is less than the number of connected configs, PAL will write up to
 * `count`.
 *
 ...
 *
 * The count must be the number of opengl framebuffers in the framebuffer
 * configs array.
 *
 ...
 *
 * The opengl system must be initialized before this call.
 * The affects the currently cound context on the calling thread, see
 * `palMakeContextCurrent()`.
 *
 * `interval` == 0, no vsync
 *
 * `interval` == 0, vsync
 ...
 * @ingroup opengl
 */
PAL_API void PAL_CALL palSetSwapInterval(Int32 interval);
