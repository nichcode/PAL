...
typedef enum {
    ...
    PAL_EVENT_MOUSE_WHEEL,             /** < The delta of the mouse position. This will be triggered if there is a focus window.*/
    PAL_EVENT_USER,                    /** < A user event. Differentiate between them with `userId` field in `PalEvent`.*/
    PAL_EVENT_MAX
} PalEventType;
...
typedef struct PalEvent {
    ...
    Int64 userId;                 /** < This is for user events. You can have user events upto Int64 max.*/
} PalEvent;
...
typedef struct {
    ...
    void* userData;       /** Optional pointer to user data passed into event queue queue functions. Can be `nullptr`.*/
} PalEventQueue;
...
typedef struct {
    ...
    void* userData;                       /** Optional pointer to user data passed into event callback. Can be `nullptr`.*/
 ...
 *
 * @param[in] info Pointer to a `PalEventDriverCreateInfo` struct with creation specifications.
 ...
 * 
 * If the dispatch mode is `PAL_DISPATCH_POLL`, the event will be dispatched to the event queue.
 * If the dispatch mode is `PAL_DISPATCH_CALLBACK` and the event driver has a valid callback,
 * the event will be dispatched to the event callback. 
 *
 ...
 * 
 * If the dispatch mode for `event` is `PAL_DISPATCH_POLL`, the event will be pushed to the event queue. 
 * Otherwise if dispatch mode is `PAL_DISPATCH_CALLBACK`, the event callback function will be called. 
 * 
 * If the dispatch mode is `PAL_DISPATCH_CALLBACK`, and the `eventDriver` has no valid event callback,
 * the function returns silently.
 *
 * @param[in] eventDriver Pointer to the event driver.
 * @param[in] event Pointer to an event. This must be valid and should be initialized and explicitly set by the user.
 *
 * @note This function is not thread-safe. If multiple threads will call this function,
 * the user is responsible for synchronization.
