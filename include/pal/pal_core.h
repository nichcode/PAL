 ...
 * 
 * Allocates `size` bytes with `alignment` and return a pointer to the memory address or nullptr on failure.
 ...
 * @param[in] alignment Must be power of two. It is recommended that it should atleast `8` or `16`. If zero `16` will be used.
 * 
 ...
 *
 * @param[in] userData Optional pointer to user data. Can be `nullptr`.
 * @param[in] ptr Pointer to the memory to free. This function should do nothing if `ptr` is `nullptr`
 * 
 ...
 * 
 * This provides control over how log messages are handled (eg. redirect the log message to a file).
 * 
 * The `msg` is only valid for the duration of the callback, copy if you need to store it.
 *
 * @param userData Optional pointer to user data. Can be `nullptr`.
 * @param msg A constant null-terminated UTF-8 encoding string containing the message.
 * 
 * @sa PalLogger, palLog()
 * @ingroup core
 */
 ...
 * 
 * `PAL_RESULT_SUCCESS` code means the operation completed successfully. Any other value indicates an error.
 * 
 * @note For clarity, always use the named constants like 
 * `PAL_RESULT_OUT_OF_MEMORY` not their numeric values.
 * 
 * @note All results follow the format `PAL_RESULT_**` for consistency and API use.
 */
typedef enum {
    ...
    PAL_RESULT_PLATFORM_FAILURE,                  /** < An error occured on the plaform (OS) level.*/
    ...
    PAL_RESULT_INSUFFICIENT_BUFFER,               /** < Buffer provided for write operation is too small.*/

    PAL_RESULT_INVALID_THREAD,                    /** < An invalid thread.*/
    PAL_RESULT_THREAD_FEATURE_NOT_SUPPORTED,      /** < The thread feature used was not supported*/
    ...
    PAL_RESULT_VIDEO_FEATURE_NOT_SUPPORTED,       /** < The video feature used was not supported.*/
    ...
    PAL_RESULT_GL_EXTENSION_NOT_SUPPORTED,        /** < The opengl extension used was not supported.*/
    PAL_RESULT_INVALID_GL_FBCONFIG,               /** < An invalid opengl framebuffer config.*/
    PAL_RESULT_INVALID_GL_VERSION,                /** < The requested opengl version is not supported.*/
    PAL_RESULT_INVALID_GL_PROFILE,                /** < The requested opengl profile is not supported.*/
...
typedef struct {
    ...
    void* userData;                  /** Optional pointer to user data passed into allocation functions. Can be `nullptr`.*/
} PalAllocator;
...
typedef struct {
    ...
    void* userData;             /** Optional pointer to user data passed into log callback. Can be `nullptr`.*/
} PalLogger;

/**
 * @brief Get the runtime version of PAL.
 * 
 * This returns a copy of PALVersion struct, which contains the major, minor and build of the PAL runtime.
 *
 ...
 *
 * This returns a constant, null-terminated UTF-8 encoding string describing the PAL runtime version
 * in the format: `1.2.3`, where `1` is the major, `2` is the minor and `3` is the build versions respectively.
 *
 ...
 * 
 * This returns a constant, null-terminated UTF-8 encoding string describing the result.
 * 
 ...
 * 
 * If a valid allocator is provided, its `allocate` function is called with the provided size and user data.
 * Otherwise, PAL's default allocator is used.
 *
 * @param[in] allocator Optional pointer to an allocator. The default allocator will be used if it is `nullptr`.
 * @param[in] size Size in bytes to allocate.
 * @param[in] alignment Must be power of two. It is recommended that it should atleast `8` or `16`. If zero `16` will be used.
 * 
 ...
 * 
 * If a valid allocator is provided, its `free` function is called with the provided ptr and user data.
 * Otherwise, PAL's default allocator is used.
 *
 ...
 *
 * If the provided logger does not have a valid `callback`, PAL will discard it and use its default one.
 * To use PAL's default logger, pass in `nullptr`. This supports unicode characters.
 *
 * @param[in] logger Optional pointer to a logger. The default logger will be used if it is `nullptr`.
