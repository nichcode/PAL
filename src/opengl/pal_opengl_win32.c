...
typedef WINBOOL (WINAPI *SwapBuffersFn)(
    HDC);

// wgl functions
typedef PROC (WINAPI *wglGetProcAddressFn)(
    LPCSTR);

typedef HGLRC (WINAPI *wglCreateContextFn)(
    HDC);

typedef BOOL (WINAPI *wglDeleteContextFn)(
    HGLRC);
...
// gl functions
typedef const GLubyte *(WINAPI *glGetStringFn)(
    GLenum);
...
typedef BOOL (WINAPI *wglSwapIntervalEXTFn)(
    int);

typedef const char* (WINAPI *wglGetExtensionsStringEXTFn)();

typedef const char* (WINAPI *wglGetExtensionsStringARBFn)(
    HDC);
